	...
	if($flash['focus_group'])
	{
	    ...
	    if($flash['results'])
		{
			...
			foreach($flash['results'] as $k=>$o)
			{
                ...
                $o->thumbpath = ispic("/upload/small.png");
			...
			return $flash;
		}else echo "<span style='color:#fff'>暂无图片。<span>";
	}else{ echo '暂无数据！';}
}
...
function doc_focus_content($group_id=0,$n=0,$style=0,$strcount=0,$strcount1=0,$isellipsis=true,$ordering='id',$fromcount=0)
{
    $opts=array(
        'hastag'=>false,
        'fun'=>'doc_focus',
    ...
    return shl_focus($group_id, $fromcount, $n, $ordering, $opts);
}
